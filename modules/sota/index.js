...
module.exports = class SOTASpots extends events.EventEmitter {
  ...
  async run(opts = {}) {
    while (true) {
      ...
      if (this.circuitOpen) {
        console.error('[SOTA] Circuit breaker OPEN - too many consecutive failures. Stopping polling.');
      ...
      try {
        ...
        if (this.consecutiveErrors >= this.MAX_CONSECUTIVE_ERRORS) {
          ...
          console.error(`[SOTA] Circuit breaker triggered after ${this.consecutiveErrors} consecutive failures`);
        ...
        console.error(`[SOTA] Fetch failed (attempt ${this.consecutiveErrors}/${this.MAX_CONSECUTIVE_ERRORS}, retry in ${nextRetrySeconds}s): ${err.message || err}`);
      }
    }
  }
  ...
  resetCircuitBreaker() {
    ...
    this.consecutiveErrors = 0;
    this.currentBackoff = 0;
    console.log('[SOTA] Circuit breaker reset');
  }
};
