...
/**
 * Parse skimmer message into structured components.
 * Only processes skimmer spots (those with -# suffix in spotter callsign).
 * Extracts mode, SNR, speed, and type from skimmer messages.
 *
 * @param {string} message - Skimmer message (e.g., "CW 34 dB 28 WPM CQ")
 * @param {string} spotter - Spotter callsign to check for -# suffix
 * @returns {object|null} Parsed skimmer data or null if not a skimmer spot or parsing fails
 *
 * @example
 * parseSkimmerMessage("CW 34 dB 28 WPM CQ", "LZ5DI-#")
 * // Returns: { mode: "CW", snr: 34, speed: 28, type: "CQ" }
 *
 * parseSkimmerMessage("tnx QRP 1W QSO 73 GL", "9Y4M")
 * // Returns: null (not a skimmer spot)
 */
function parseSkimmerMessage(message, spotter) {
    // Only process skimmer spots (those with -# suffix)
    if (!spotter || !spotter.includes('-#')) {
        return null;
    }
    ...
    // Pattern: MODE SNR dB SPEED WPM TYPE
    // Example: "CW 34 dB 28 WPM CQ" or "CW  4 dB 18 WPM BEACON"
    const regex = /^(\w+)\s+(\d+)\s+dB\s+(\d+)\s+WPM\s+(\w+).*$/;
    ...
    return {
        mode: match[1],           // e.g., "CW", "RTTY"
        snr: parseInt(match[2], 10),  // e.g., 34 (without "dB")
        speed: parseInt(match[3], 10),// e.g., 28 (without "WPM")
        type: match[4]            // e.g., "CQ", "BEACON"
    };
}
