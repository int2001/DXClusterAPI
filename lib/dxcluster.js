...
module.exports = class DXCluster extends events.EventEmitter {
  ...
  connect(opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      // Store custom command from connection options
      this.customCommand = opts.customCommand || null
      ...
      this.socket.on('error', (err) => {
        ...
        if (connectionTimedOut) {
          // Connection already timed out, cleanup resources
          this._cleanupResources();
          return; // Ignore if already timed out
        }
      ...
      this.socket.on('data', (data) => {
        ...
        if(this.status.awaiting_login) {
          if(dataStr.indexOf(loginPrompt) != -1) {
            if(this.write(call)) {
              this.status.awaiting_login = false
              // Execute custom command after login if no password required
              if (!opts.password) {
                this._executeCustomCommand()
              }
            }
          }
        }
        if(this.status.awaiting_password) {
          if(dataStr.indexOf(passPrompt) != -1) {
            if(this.write(opts.password)) {
              this.status.awaiting_password = false
              // Execute custom command after password if configured
              this._executeCustomCommand()
            }
          }
        }
        this._parseDX(dataStr)
      })
  ...
  close() {
    ...
    if (this.socket) {
      this.socket.end();
      // Call cleanup to prevent memory leaks
      this._cleanupResources();
    }
    this.emit('closed')
  }
  ...
  /**
   * Execute custom command after authentication is complete
   * @private
   */
  _executeCustomCommand() {
    if (this.customCommand) {
      this.write(this.customCommand)
      this.emit('custom_command_sent', this.customCommand)
    }
  }

  /**
   * Internal method to cleanup all resources and prevent memory leaks
   * Called during connection failures, timeouts, and shutdown
   * @private
   */
  _cleanupResources() {
    if (this.socket) {
      ...
      // Clear socket reference to prevent dangling references
      this.socket = null;
    }

    // Reset all connection status flags
